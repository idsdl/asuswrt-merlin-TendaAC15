...
our %config = (
  ...
  CFLAGS => [ "-Os -g -DLINUX26 -DCONFIG_BCMWL5 -DDEBUG_NOISY -DDEBUG_RCTEST -pipe -DBCMWPA2 -DBCMARM -fno-strict-aliasing -marm -DRTCONFIG_NVRAM_64K -DLINUX_KERNEL_VERSION=132644 ", "-O2", "-O2", "-ffunction-sections", "-fdata-sections" ],
  CPPDEFINES => [  ],
  CPPFLAGS => [  ],
  CPPINCLUDES => [  ],
  CROSS_COMPILE => " ",
  CXX => "arm-brcm-linux-uclibcgnueabi-g++",
  CXXFLAGS => [ "-Wall -O3", "-O2", "-O2", "-ffunction-sections", "-fdata-sections" ],
  ...
  cppflags => [  ],
  cxxflags => [  ],
  defines => [ "NDEBUG", "OPENSSL_API_COMPAT=0x10000000L" ],
  ...
  libdir => "",
  major => "1",
  makedepprog => "\$(CROSS_COMPILE)arm-brcm-linux-uclibcgnueabi-gcc",
  minor => "1.1",
  openssl_algorithm_defines => [ "OPENSSL_NO_ARIA", "OPENSSL_NO_MD2", "OPENSSL_NO_RC4", "OPENSSL_NO_SM2", "OPENSSL_NO_SM3", "OPENSSL_NO_SM4" ],
  openssl_api_defines => [ "OPENSSL_MIN_API=0x10000000L" ],
  ...
  openssldir => "/etc/ssl",
  options => "-O2 --prefix=/usr --openssldir=/etc/ssl --cross-compile-prefix=  --api=1.0.0 -O2 -ffunction-sections -fdata-sections -Wl,--gc-sections enable-shared enable-rc5 no-afalgeng no-aria no-asan no-async no-buildtest-c++ no-crypto-mdebug no-crypto-mdebug-backtrace no-devcryptoeng no-ec_nistp_64_gcc_128 no-egd no-err no-external-tests no-fuzz-afl no-fuzz-libfuzzer no-heartbeats no-md2 no-msan no-rc4 no-sctp no-sm2 no-sm3 no-sm4 no-ssl-trace no-ssl3 no-ssl3-method no-ubsan no-unit-test no-weak-ssl-ciphers no-zlib no-zlib-dynamic",
  perl_archname => "x86_64-linux-thread-multi",
  perl_cmd => "/usr/bin/perl",
  perl_version => "5.26.3",
  perlargv => [ "linux-armv4", "-O2", "--prefix=/usr", "--openssldir=/etc/ssl", "--cross-compile-prefix= ", "--api=1.0.0", "-O2", "-ffunction-sections", "-fdata-sections", "-Wl,--gc-sections", "shared", "no-err", "no-async", "enable-rc5", "no-rc4", "no-ssl3", "no-aria", "no-sm2", "no-sm3", "no-sm4" ],
  perlenv => {
      "AR" => "arm-brcm-linux-uclibcgnueabi-ar",
      "BUILDFILE" => undef,
      "CC" => "arm-brcm-linux-uclibcgnueabi-gcc",
      "CFLAGS" => "-Os -g -DLINUX26 -DCONFIG_BCMWL5 -DDEBUG_NOISY -DDEBUG_RCTEST -pipe -DBCMWPA2 -DBCMARM -fno-strict-aliasing -marm -DRTCONFIG_NVRAM_64K -DLINUX_KERNEL_VERSION=132644 ",
      "CPPFLAGS" => undef,
  ...
  sdirs => [ "objects", "md4", "md5", "sha", "mdc2", "hmac", "ripemd", "whrlpool", "poly1305", "blake2", "siphash", "des", "aes", "rc2", "rc5", "idea", "bf", "cast", "camellia", "seed", "chacha", "modes", "bn", "ec", "rsa", "dsa", "dh", "dso", "engine", "buffer", "bio", "stack", "lhash", "rand", "err", "evp", "asn1", "pem", "x509", "x509v3", "conf", "txt_db", "pkcs7", "pkcs12", "comp", "ocsp", "ui", "cms", "ts", "srp", "cmac", "ct", "async", "kdf", "store" ],
  shlib_major => "1",
  shlib_minor => "1",
